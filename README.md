...
| 代码解析 | JavaParser |
| Git 操作 | JGit |
| 向量化 | 通义等兼容 OpenAI 格式的 Embedding API |
...
| `EmbeddingApiClient` | — | 外部 Embedding REST API |
...
IndexController.triggerIndex（虚拟线程异步提交）
  └─ FullIndexService.runFullIndex(repoId)
       ...
       │         ├─ EmbeddingApiClient      → 外部 Embedding API（HTTP）
...
ImpactController.semanticSearch
  └─ EmbeddingStoreAdapter.semanticSearch
       ├─ EmbeddingApiClient       → 外部 Embedding API（将查询文本向量化）
